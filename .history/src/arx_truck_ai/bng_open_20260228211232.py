...
def is_coupled(bng, truck_vid, trailer_vid=None):
    ...
    print("truck_info:", truck_info, flush=True)

    if not truck_info:
        return False

    couplers = truck_info.get('couplers', {}) or {}
    for tag, data in couplers.items():
        state = (data.get('state') or '').lower()
        if state not in ('coupled', 'locked', 'attached'):
            continue
        if trailer_vid and data.get('otherVehicle') != trailer_vid:
            continue
        return True
    return False
...
def main():
    ...
    orig          = (-50.0, -200.0, 0) # x = -50, y = -200, z = 0 Para un buen punto en autotest con el camión
    trailer_orig  = (-49.0, -204.7, 0)  # 5.5 metros detrás
    rot_quat      = (0, 0, 1, 0)
