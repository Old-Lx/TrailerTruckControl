...
class TruckTrailer:
    ...
    def __init__(self, truck: Vehicle, trailer: Vehicle, bng: BeamNGpy):
        ...
        self.bng = bng
        self.state_truck = State('state_truck', self.bng, self.truck)
        self.state_trailer = State('state_trailer', self.bng, self.trailer)
    ...
    def read_sensors(self):
        print("Hola")
        # Leer datos de los sensores avanzados directamente
        ...
        truck_state = self.state_truck.poll()
        trailer_state = self.state_trailer.poll()
        ...
        psi_truck = np.arctan2(truck_state['dir'][1], truck_state['dir'][0])
        psi_trailer = np.arctan2(trailer_state['dir'][1], trailer_state['dir'][0])
        delta_F2 = psi_truck - psi_trailer

        # Las imágenes vienen en formato PIL o RGBA
        frame = reverse_cam_data['colour']
        ...
        if guard_points.size > 0: # Prevenir error si el array está vacío en el primer frame
            min_radius = np.min(np.linalg.norm(guard_points[:, :2], axis=1))
            # Asumiendo que resuelves v1 arriba
