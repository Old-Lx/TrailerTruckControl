...
# No he logrado conectar el trailer sin rodarlo hacia atrás primero, este es un bypass (espero que temporal)
def attach_trailer(truck: Vehicle):
    truck.control(gear = -1, throttle = 0.1)
    time.sleep(10)
    truck.control(gear=1)
def main():
    bng, truck, orig = bng_open.main()
    ...
    attach_trailer(truck)

    # Make the truck's AI span the map
    # truck.ai.set_script(script)
