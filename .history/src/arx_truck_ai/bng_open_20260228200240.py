...
local states = obj:getCouplerState()
if states then
  for tag, state in pairs(states) do
    if state and state.nodeId and state.otherNodeId then
      attached = true
      break
    end
  end
...
def is_coupled(vehicle):
    resp = vehicle.queue_lua_command(CHECK_COUPLER_LUA, response=True)
...
def main():
    ...
    for _ in range(120):        # dos segundos aprox
        truck.control(gear=-1, throttle=0.15)
        bng.step(0.016)

    truck.control(throttle=0, brake=1)
