...
class TruckTrailer:
    ...
    def read_sensors(self):
        truck_data = self.bng.poll_sensors(self.truck)
        trailer_data = self.bng.poll_sensors(self.trailer)

        v1 = truck_data['imu_main']['state']['vel'][0]
        psi_truck = self.truck.state['yaw']
        psi_trailer = self.trailer.state['yaw']
    ...
    def set_sensors(self):
        ...
        self.trailer.attach_sensor('line_cam', Camera(pos=(0, 0, 2.0), dir=(0, 0, -1), fov=70, resolution=(320, 180), colour=True, is_render_colours=True))
...
def gen_truck_and_trailer(scenario, bng):
    ...
    trailer_orig  = (-3.017, 0, 1.204) # 0.949 Esta es la ubicación que debe tener el trailer para estar exactamente en el punto donde se puede conectar el camión, esto corresponde a -3.017 a lo largo el eje de movimiento y 0.949 a lo largo del eje de giro
