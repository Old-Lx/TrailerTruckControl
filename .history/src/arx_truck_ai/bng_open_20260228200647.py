...
COUPLER_CHUNK = """
local veh = scenetree.findObject("%s")
if not veh then return false end
local states = veh:getCouplerState()
if not states then return false end
for _, s in pairs(states) do
  if s.state == "coupled" or (s.otherNodeId and s.nodeId) then
    return true
  end
end
return false
...
def is_coupled(bng, vehicle):
    chunk = COUPLER_CHUNK % vehicle.vid
    resp = bng.queue_lua_command(chunk, response=True)  # GE context
    return bool(resp)
...
def main():
    ...
    while not is_coupled(bng, truck):
