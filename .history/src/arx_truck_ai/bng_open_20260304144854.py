...
def gen_truck_and_trailer(scenario, bng):
    ...
    trailer_base_cfg = trailer_configs['28ft_9600kg'] 
    ...
    orig          = (0, 0, 0.738) # x = -50, y = -200, z = 0 Para un buen punto en autotest con el camión
    trailer_orig  = (0.949, -3.017, 1.204)
    rot_quat      = (0, 0, 1, 0)
...
def detect_vehicles(bng: BeamNGpy):
    ...
    print("Detectados:", {vid: v.options.get("model") for vid, v in vehicles.items()})
    truck = next((v for v in vehicles.values() if v.options.get('model') == 'semi'), None)
    if truck is None:
        raise RuntimeError("No se encontró ningún vehículo modelo 'semi'")
...
def main():
    ...
    orig, truck =  gen_truck_and_trailer(scenario, bng) #  detect_vehicles(bng)
