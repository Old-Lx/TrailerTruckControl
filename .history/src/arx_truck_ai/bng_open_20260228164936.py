...
def main():
    ...
    available = bng.get_available_vehicles()
    t_configs = available['vehicles']['t-series']['configs']
    for name, cfg in t_configs.items():
        print(name, cfg['config'])  # 'config' apunta al .pc
    ...
    cement_pc = t_configs['T-Series Cement Mixer']['config']
    truck = Vehicle('truck2', model='t-series', part_config=cement_pc)
